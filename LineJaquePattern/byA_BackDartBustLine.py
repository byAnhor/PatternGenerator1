...
class byA_BackDartBustLine(byA_FrozenClass):
     ...
     def __init__(self,**kwargs):
        ...
        if (self._parent is not None):
           self._parent._dicoPoints['BackDartBustLineMark_mark'] = self._mark
        self._freeze("byA_BackDartBustLine")
     ...
     def addToGroup(self, drawing, svggroup, **extra):
        ...
        nomenclature = drawing.text(self._nomenclature, id="nomenclatureBackDartBustLine"+self._stature, insert=(self._mark._x, self._mark._y))
        svggroup.add(nomenclature)
        drawing.save()
        textw=PXCM * float(subprocess.check_output(["C:\\Program Files\\Inkscape\\inkscape.exe",
                          "--query-id=nomenclatureBackDartBustLine"+self._stature, "--query-width", self._filename]))
