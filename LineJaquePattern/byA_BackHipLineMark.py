...
class byA_BackHipLineMark(byA_FrozenClass):
     ...
     def __init__(self,**kwargs):
        ...
        tmpMark = self._parent._dicoPoints['BackHipLineMark_tmpMark']
        self._mark = byA_Point(x=tmpMark._x+(bassin - bustCompare)/2.0, y=tmpMark._y) 
        self._nomenclature = "D'"
        if (self._parent is not None):
           self._parent._dicoPoints['BackHipLineMark_mark'] = self._mark
     ...
     def addToGroup(self, drawing, svggroup, **extra):
        ...
        nomenclature = drawing.text(self._nomenclature, id="nomenclatureBackHipLineMark"+self._stature, insert=(self._mark._x, self._mark._y))
        svggroup.add(nomenclature)
        drawing.save()
        textw=PXCM * float(subprocess.check_output(["C:\\Program Files\\Inkscape\\inkscape.exe",
                          "--query-id=nomenclatureBackHipLineMark"+self._stature, "--query-width", self._filename]))
